...
async function loadConfig() {
  try {
    const res = await props.api.get('plugin/PresetRename/get_config');
    if (res.data) {
...
async function updatePreview() {
  try {
    ...
    const res = await props.api.get(`plugin/PresetRename/get_preview?${params}`);
...
async function saveConfig() {
  saving.value = true;
  try {
    const res = await props.api.post('plugin/PresetRename/save_config', config);
    if (res.data && res.data.success) {
        ...
        _createVNode(_component_v_card_text, { class: "pt-0" }, {
          default: _withCtx(() => [
            _createElementVNode("div", _hoisted_2, [
              ...
              _createElementVNode("code", _hoisted_3, _toDisplayString(config.current_movie_format || '未设置'), 1)
            ]),
            _createElementVNode("div", _hoisted_4, [
              _cache[8] || (_cache[8] = _createElementVNode("span", { class: "format-label" }, "剧集：", -1)),
              _createElementVNode("code", _hoisted_5, _toDisplayString(config.current_tv_format || '未设置'), 1)
            ])
          ]),
          _: 1
        })
...
const Config = /*#__PURE__*/_export_sfc(_sfc_main, [['__scopeId',"data-v-3eab4792"]]);
