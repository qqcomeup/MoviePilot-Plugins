...
# from app.helper.wallpaper import WallpaperHelper  # 旧版引用，已注释
from app.helper.image import ImageHelper  # 新版引用
    ...
    def _get_matched_image(self, title: str, text: str) -> Tuple[str, str]:
        ...
        for mapping in self._image_mappings:
            ...
            if keyword.lower() in title.lower() or keyword.lower() in text.lower():
                ...
                resolved_urls = []
                if image_urls:
                    # --- 修改开始: 使用 ImageHelper 替代 WallpaperHelper ---
                    helper = ImageHelper()
                    for u in image_urls:
                        if u == "背景壁纸":
                            try:
                            ...
                            try:
                                # 注意：如果 ImageHelper 没有 get_wallpapers 方法，可能需要调整
                                # 通常 refactor 会保留同名方法或提供等价方法
