...
- 需要通过机器人重置 TVH 用户 Token，或临时启用/禁用用户。
...
- TVH 管理员账号/密码：用于读取 TVH 用户、状态、修改用户和关闭连接。
...
TVH 本身在 MoviePilot 插件生态里属于相对冷门的 IPTV/电视直播管理场景。本插件主要用于把 MoviePilot 机器人和 TVHeadend 连接起来，方便管理 TVH 用户链接、在线播放连接、用户 Token 和 DVB 状态。
